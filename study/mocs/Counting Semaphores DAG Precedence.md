...
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **V1** | **Asymmetric Diamond:**<br><br>  <br>  <br><br>$T_1 \to \{T_2, T_3, T_4\}$;<br><br>  <br>  <br><br>$\{T_2, T_3\} \to T_5$; $T_4$ independent of $T_5$. | **2 counting semaphores**<br><br>  <br>  <br><br>$S_1=0$ for fork from $T_1$ ($3 \times V$).<br><br>  <br>  <br><br>$S_2=0$ for join into $T_5$ ($2 \times P$). |
| **V2** | **Formula Derivation:**<br><br>  <br>  <br><br>Init = $k$, Signals = $n$, Attempts = $m$, Blocked = $w > 0$. Find completed $P$.                       | **$\#P_{\text{completed}} = m - w = k + n$**<br><br>  <br>  <br><br>_(Note: $m = k + n + w$)_                                                                   |
| **V3** | **Join Independence:**<br><br>  <br>  <br><br>$A \to C$ and $B \to C$ via shared semaphore $S=0$. Can $C$ run with only one finished?                  | **No (Guaranteed correct)**<br><br>  <br>  <br><br>$C$ requires 2 tokens ($P(S), P(S)$). Since $A$ and $B$ emit 1 token each, both must finish.                 |
| **V4** | **Hourglass DAG ($3 \to 1$):**<br><br>  <br>  <br><br>$\{T_1, T_2, T_3\} \to T_4$. Minimal semaphores for Counting vs. Binary.                         | **Counting = 1** (Successor calls $3 \times P$)<br><br>  <br>  <br><br>**Binary = 3** (1 per dependency edge to prevent lost signals)                           |

```

You can download the raw `.md` / `.txt` file directly below:

* **Download Note:** [counting_semaphores_dag_summary.txt](sandbox:/counting_semaphores_dag_summary.txt)
```