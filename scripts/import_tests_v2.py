...
for sheet_name, rel_folder in folder_map.items():
    ...
    for r in range(2, sheet.max_row + 1):
        if sheet_name == 'CSE quiz':
            ...
            tname_cell = sheet.cell(r, 3)
            qs = sheet.cell(r, 4).value
            marks = sheet.cell(r, 5).value
            duration = sheet.cell(r, 6).value
            taken = sheet.cell(r, 7).value

        tname = tname_cell.value
        url = ""
        if tname_cell.hyperlink:
            url = tname_cell.hyperlink.target
            # Fix relative URLs from gateoverflow
            if url.startswith("https://gateoverflow.in/exams/../"):
                url = url.replace("https://gateoverflow.in/exams/../", "https://gateoverflow.in/")
            elif url.startswith("https://gateoverflow.in/./"):
                url = url.replace("https://gateoverflow.in/./", "https://gateoverflow.in/")
...
- **Status**: `{status_str}`
- 🔗 **Online Test Link**: [{tname_str}]({url})
...
print(f"Successfully updated {created_count} test notes with embedded URLs and done flags in lowercase.")
